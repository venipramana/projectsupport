...
@section('header-title', 'Project Administration')
...
<div class="table-container">
    <table class="data-table">
        ...
        <tbody>
            @forelse($rprojects as $proj)
            <tr>
                <td>{{ $proj->id }}</td>
                <td>{{ $proj->deskripsi }}</td>
                <td>
                    @if($proj->icon)
                        <i class="{{ $proj->icon }}" style="font-size: 1.2rem; color: var(--primary);"></i> ({{ $proj->icon }})
                    @else
                        -
                    @endif
                </td>
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn-icon btn-edit" onclick="openEditModal({{ $proj->id }}, '{{ addslashes($proj->deskripsi) }}', '{{ addslashes($proj->icon ?? '') }}', {{ $proj->status ?? 0 }})" title="Edit">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </button>
                        <form action="{{ route('rproject.destroy', $proj->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus project ini?');">
                            @csrf
                            @method('DELETE')
...
<div id="createModal" class="modal">
    <div class="modal-content">
        ...
        <form action="{{ route('rproject.store') }}" method="POST">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label>Deskripsi (Nama Project)</label>
                    <input type="text" name="deskripsi" class="form-control" required>
                </div>
                <div class="form-group">
                    <label>Icon (misal: fa-solid fa-folder)</label>
...
<script>
    ...
    function openEditModal(id, deskripsi, icon, status) {
        // Set form action dynamically
        const form = document.getElementById('editForm');
        form.action = `/rproject/${id}`;
